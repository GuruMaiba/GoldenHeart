...
// $('.main-right').scrolly();
    new WOW().init();
$(".main-index").css({height:height + "px"});
 window.onscroll = function() {
        var scrolled = window.pageYOffset || document.documentElement.scrollTop;
        $(".main-right").css({top:(150 - scrolled) + "px"});
    }
});
function SetHoliday ()
{
...
{
 $(".mask").css({display: "block"});
 $(".go-message").fadeIn(1000);
$(".mask").click(function()
{
$(".mask").css({display: "none"});
 $(".go-message").fadeOut(1000);
 $(".blur-fon").css({filter: "", background: ""});
